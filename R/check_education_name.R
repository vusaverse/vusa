...
check_education_name <- function(df, actual_edu_names = NULL, col_edu_names = NULL) {
  ...
  if (is.null(col_edu_names)) {
    message("col_edu_names not given, so look for system variables")
  ...
  if (is.null(actual_edu_names)) {
    message("actual_edu_names not given, so look for system variables")
    if (!Sys.getenv("EDU_DATA") == "") {
      message("system variables present, so these wil be used for actual_edu_names")
  ...
  if (any(!Edu_x %in% actual_edu_names)) {
    ...
    ## message the training names that are not in actual_edu_names
  ...
  ## if the previous condition is not true, the test is passed
    ...
    message("Test is passed, all education names are correct")
  }
}
