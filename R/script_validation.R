  ...
  if (export_naar_dataframe == FALSE) {
    ...
    if (all(is.na(errors$warning)) && all(is.na(errors$error))) {
      ...
      base::cat(cli::style_bold(
        cli::col_red("Niet geslaagd. Zie bericht(en) hierboven.\n")
      ))
    }
  } else {
    # Sla warning en error messages op in object
    errors <-
      # catchToList(invisible(eval(parse(text = lines), envir = globalenv())))
...
validate_style <-
  function(filepath, export_naar_dataframe = FALSE) {
    ...
    dfVariabel_names <- dplyr::as_tibble(lintr::lint(
      filepath,
      variable_name_lint()
    ))
...
make_lint_object <- function(expr, source_file) {
  name <- strip_names(as.character(xml2::xml_find_first(expr,
                                                        "./text()")))
  ...
  lintr::Lint(
    ...
    message = lint_msg,
    ## Deze variabelen zijn voor zover ik weet overbodig
    #type = "style",
